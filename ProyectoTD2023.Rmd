...
## Comprobación de data frames (NA)
```{r}
all(is.na(df_HR1))
all(is.na(df_HR2))
all(is.na(df_MHR))
...
all(is.na(df_Pm))
all(is.na(df_all))
all(is.na(df_all_melt))
# Como podemos observar, los data frame se han denerado correctamente sin ningún NA.
```
...
1. ¿Existe alguna relación entre la frecuencia cardíaca fetal y la materna?
Según lo observado en los datos, se intuye una relación entre ambas frecuencias. Se aprecia que en la medida en la que la frecuencia cardíaca materna aumenta, lo hace también la del feto. Sucede lo mismo cuando la frecuencia cardíaca materna disminuye.


2. ¿Existe alguna relación entre la saturación de oxígeno materno y la frecencia cardíaca fetal?
En las madres que presentan una saturación de oxígeno menor, la frecuencia cardíaca fetal es levemente superior que en las madres con una saturación de oxígeno un poco más elevada.

Con respecto a la relación entre las **contracciones uterinas** y la **frecuencia cardíaca fetal**, proponemos:

3. ¿Hay alguna relación entre la frecuencia y duración de las contracciones uterinas y la frecuencia cardíaca fetal?
Atendiendo a lo observado en los datos, podemos intuir que la frecuencia (y duración) de las contracciones uterinas tiene una relación con la frecuencia cardíaca fetal. Si la frecuencia de las contracciones uterinas es intensa se produce una disminución en la frecuencia cardíaca fetal.

4. ¿Hay alguna correlación entre la frecuencia cardíaca fetal y la intensidad de las contracciones uterinas durante el parto?
Al igual que en la pregunta plateada anteriormente y según los datos observados, se sospecha de una relación entre la intensidad de las contracciones uterinas y la frecuencia cardíaca fetal: si las primeras son muy intensas los valores de la frecuencia cardíaca fetal disminuyen.

En lo referido a la relación entre la **frecuencia cardíaca fetal** y las **presiones sistólica, diastólica** y la **frecuencia cardíaca materna**, proponemos:

5. ¿Hay alguna diferencia en la frecuencia cardíaca fetal entre mujeres con diferentes niveles de presión arterial o frecuencia cardíaca materna durante el parto?
Se intuye, según lo observado, que unos valores de mayor presión sanguínea materna pueden provocar una disminución de la frecuencia cardíaca fetal. Cuando los primeros son elevados la frecuencia cardíaca fetal es menor.

6. ¿Cuál es el intervalo normal de frecuencia cardíaca fetal?¿Y de frecuencia cardíaca materna?
La frecuencia cardíaca fetal considerada normal se encuentra entre los 110 y 160 latidos por minuto. Y la frecuencia cardíaca materna como norma general, ronda entre los 85-90 latidos por minuto

7. ¿Existe alguna relación entre la frecuencia cardíaca fetal y la presión sistólica o diastólica de la madre durante el parto?
Según lo observado, y tal y como se ha sugerido en la pregunta 5, se intuye la existencia de una relación entre ambas frecuencias. Unos valores de mayor presión sanguínea materna pueden provocar una disminución de la frecuencia cardíaca fetal.